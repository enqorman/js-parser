...
export function print_identifier(str: string, identifier: Identifier): string {
	str += `Identifier(name=${
		identifier.name
	}, location=(${identifier.location.toString()}))`;
	return str;
}
...
export function print_literal(str: string, literal: Literal): string {
	str += `Literal(name=${
		literal.value
	}, location=(${literal.location.toString()}))`;
	return str;
}
...
export function print_expression(str: string, expression: Expression): string {
	if (expression instanceof Identifier)
		...
		str += `TODO[print_expression]: ${expression.class_name()}`;
	}
	return str;
}

export function print_expression_statement(
	str: string,
...
export function print_function_declaration_statement(
	str: string,
	func_dcl_statement: FunctionDeclarationStatement,
): string {
	str = print_indent(str);
...
export function print_statement(str: string, statement: Statement): string {
	if (statement instanceof ExpressionStatement) {
		str = print_indent(str);
		return print_expression_statement(
			str,
		...
		str = print_function_declaration_statement(
			str,
			statement as FunctionDeclarationStatement,
		);
	} else {
		str += `TODO[print_statement]: ${statement.class_name()}`;
	}
	return str;
}
